					...
					>
						{Object.values(Currencies).map((currency) => (
							<SelectItem className="flex items-center" key={currency.code} value={currency.code}>
								<span className="text-blue-600 text-xl font-semibold mr-2">{currency.symbol}</span> {currency.label}
							</SelectItem>
						))}
					</Select>
				</div>
            ...
            <span
             ...
             {transaction.text}
						...
						<Button size="sm"
							...
							<span className="text-2xl icon-[solar--trash-bin-minimalistic-2-outline]"></span>
						</Button>
					</div>
				))}
			</div>
